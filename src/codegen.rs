...
use inkwell::values::{BasicValueEnum, FunctionValue, PointerValue, ValueKind, BasicMetadataValueEnum};
...
impl<'ctx> Codegen<'ctx> {
    ...
    pub fn compile(&mut self, program: &Program) -> Result<()> {
        ...
        for decl in &program.declarations {
            match &decl.node {
                ...
                Decl::Function(func) => { self.declare_function(func)?; }
                Decl::Enum(_) => {} // TODO
    ...
    fn declare_class(&mut self, class: &ClassDecl) -> Result<()> {
        ...
        self.classes.insert(class.name.clone(), ClassInfo { struct_type, field_indices });
    ...
    fn declare_class_constructor(&mut self, class: &ClassDecl) -> Result<()> {
        ...
        let param_types: Vec<BasicMetadataTypeEnum> = constructor.params
            .iter()
    ...
    fn compile_constructor(&mut self, class: &ClassDecl, constructor: &Constructor) -> Result<()> {
        ...
        for (i, param) in constructor.params.iter().enumerate() {
            ...
            let alloca = self.builder.build_alloca(self.llvm_type(&param.ty), &param.name).unwrap();
            self.builder.build_store(alloca, llvm_param).unwrap();
            self.variables.insert(param.name.clone(), Variable { ptr: alloca, ty: param.ty.clone() });
        }
        ...
        let ptr = self.builder.build_call(malloc, &[size.into()], "instance").unwrap();
        ...
        // Store 'this'
        let this_alloca = self.builder.build_alloca(
            self.context.ptr_type(AddressSpace::default()),
            "this"
        ).unwrap();
        self.builder.build_store(this_alloca, instance).unwrap();
        self.variables.insert("this".to_string(), Variable {
            ptr: this_alloca,
    ...
    fn compile_method(&mut self, class: &ClassDecl, method: &FunctionDecl) -> Result<()> {
        ...
        if self.needs_terminator() {
            match &method.return_type {
                Type::None => { self.builder.build_return(None).unwrap(); }
    ...
    fn compile_stmt(&mut self, stmt: &Stmt) -> Result<()> {
        match stmt {
            Stmt::Let { name, ty, value, mutable: _ } => {
                ...
                self.variables.insert(name.clone(), Variable { ptr: alloca, ty: ty.clone() });
            }
            ...
            Stmt::Return(value) => {
                ...
                let is_main = self.current_function
            ...
            Stmt::Break => {
                if let Some(loop_ctx) = self.loop_stack.last() {
                    self.builder.build_unconditional_branch(loop_ctx.after_block).unwrap();
                }
            }

            Stmt::Continue => {
                if let Some(loop_ctx) = self.loop_stack.last() {
                    self.builder.build_unconditional_branch(loop_ctx.loop_block).unwrap();
                }
            }
    ...
    fn compile_if(&mut self, cond: &Spanned<Expr>, then_block: &Block, else_block: Option<&Block>) -> Result<()> {
    ...
    fn compile_while(&mut self, cond: &Spanned<Expr>, body: &Block) -> Result<()> {
        ...
        self.loop_stack.push(LoopContext { loop_block: cond_bb, after_block: after_bb });
        for stmt in body {
            self.compile_stmt(&stmt.node)?;
        }
        self.loop_stack.pop();
    ...
    fn compile_for(&mut self, var: &str, var_type: Option<&Type>, iterable: &Spanned<Expr>, body: &Block) -> Result<()> {
        ...
        self.variables.insert(var.to_string(), Variable { ptr: var_alloca, ty: ty.clone() });
        ...
        // Condition
        self.builder.position_at_end(cond_bb);
        let current = self.builder.build_load(self.context.i64_type(), var_alloca, var).unwrap().into_int_value();
        let cond = self.builder.build_int_compare(IntPredicate::SLT, current, end_val, "cmp").unwrap();
        self.builder.build_conditional_branch(cond, body_bb, after_bb).unwrap();

        // Body
        self.builder.position_at_end(body_bb);
        self.loop_stack.push(LoopContext { loop_block: inc_bb, after_block: after_bb });
        for stmt in body {
            self.compile_stmt(&stmt.node)?;
        }
        self.loop_stack.pop();
        ...
        for arm in arms {
            ...
            match &arm.pattern {
                ...
                Pattern::Ident(binding) => {
                    ...
                    self.builder.build_store(alloca, val).unwrap();
                    self.variables.insert(binding.clone(), Variable { ptr: alloca, ty: Type::Integer });
                ...
                Pattern::Variant(variant_name, bindings) => {
                    ...
                    match variant_name.as_str() {
                        "Some" => {
                            ...
                            // Check tag == 1
                            let tag = self.builder.build_extract_value(val.into_struct_value(), 0, "tag").unwrap();
                            let cond = self.builder.build_int_compare(
                                IntPredicate::EQ,
                                tag.into_int_value(),
                            ...
                            if !bindings.is_empty() {
                                let inner_val = self.builder.build_extract_value(val.into_struct_value(), 1, "inner").unwrap();
                                let alloca = self.builder.build_alloca(inner_val.get_type(), &bindings[0]).unwrap();
                                self.builder.build_store(alloca, inner_val).unwrap();
                                self.variables.insert(bindings[0].clone(), Variable { ptr: alloca, ty: Type::Integer });
                            }
                        ...
                        "Ok" => {
                            ...
                            if !bindings.is_empty() {
                                let ok_val = self.builder.build_extract_value(val.into_struct_value(), 1, "ok").unwrap();
                                let alloca = self.builder.build_alloca(ok_val.get_type(), &bindings[0]).unwrap();
                        ...
                        "Error" => {
                            ...
                            if !bindings.is_empty() {
                                let err_val = self.builder.build_extract_value(val.into_struct_value(), 2, "err").unwrap();
                                let alloca = self.builder.build_alloca(err_val.get_type(), &bindings[0]).unwrap();
                                self.builder.build_store(alloca, err_val).unwrap();
                                self.variables.insert(bindings[0].clone(), Variable { ptr: alloca, ty: Type::String });
                            }
        ...
        match expr {
            ...
            Expr::Ident(name) => {
                if let Some(var) = self.variables.get(name) {
                    let val = self.builder.build_load(self.llvm_type(&var.ty), var.ptr, name).unwrap();
                    Ok(val)
                } else {
            ...
            Expr::Binary { op, left, right } => {
                self.compile_binary(*op, &left.node, &right.node)
            }

            Expr::Unary { op, expr } => {
                self.compile_unary(*op, &expr.node)
            }
            ...
            Expr::Field { object, field } => {
                self.compile_field(&object.node, field)
            }

            Expr::Index { object, index } => {
                self.compile_index(&object.node, &index.node)
            }

            Expr::Construct { ty, args } => {
                self.compile_construct(ty, args)
            }
            ...
            Expr::Match { expr, arms } => {
                self.compile_match_expr(&expr.node, arms)
            }

            Expr::Try(inner) => {
                self.compile_try(&inner.node)
            }
            ...
            Expr::Require { condition, message } => {
                ...
                let current_fn = self.current_function.ok_or(
                    CodegenError::new("require outside of function")
                )?;
                ...
                self.builder.build_conditional_branch(cond, assert_block, fail_block).unwrap();
    ...
    fn compile_if_expr(
        ...
        let current_fn = self.current_function.ok_or(
            CodegenError::new("if expression outside of function")
        )?;
        ...
        if then_result.get_type() == else_result.get_type() {
            let phi = self.builder.build_phi(then_result.get_type(), "if.result").unwrap();
    ...
    fn compile_binary(&mut self, op: BinOp, left: &Expr, right: &Expr) -> Result<BasicValueEnum<'ctx>> {
        ...
        if lhs.is_int_value() && rhs.is_int_value() {
            ...
            let result = match op {
                ...
                BinOp::NotEq => self.builder.build_int_compare(IntPredicate::NE, l, r, "ne").unwrap(),
                BinOp::Lt => self.builder.build_int_compare(IntPredicate::SLT, l, r, "lt").unwrap(),
                BinOp::LtEq => self.builder.build_int_compare(IntPredicate::SLE, l, r, "le").unwrap(),
                BinOp::Gt => self.builder.build_int_compare(IntPredicate::SGT, l, r, "gt").unwrap(),
                BinOp::GtEq => self.builder.build_int_compare(IntPredicate::SGE, l, r, "ge").unwrap(),
        ...
        if lhs.is_float_value() && rhs.is_float_value() {
            ...
            let result = match op {
                ...
                BinOp::NotEq => self.builder.build_float_compare(FloatPredicate::ONE, l, r, "fne").unwrap().into(),
                BinOp::Lt => self.builder.build_float_compare(FloatPredicate::OLT, l, r, "flt").unwrap().into(),
                BinOp::LtEq => self.builder.build_float_compare(FloatPredicate::OLE, l, r, "fle").unwrap().into(),
                BinOp::Gt => self.builder.build_float_compare(FloatPredicate::OGT, l, r, "fgt").unwrap().into(),
                BinOp::GtEq => self.builder.build_float_compare(FloatPredicate::OGE, l, r, "fge").unwrap().into(),
    ...
    fn compile_call(&mut self, callee: &Expr, args: &[Spanned<Expr>]) -> Result<BasicValueEnum<'ctx>> {
        ...
        if let Expr::Field { object, field } = callee {
            if let Expr::Ident(type_name) = &object.node {
                match (type_name.as_str(), field.as_str()) {
                    ("Option", "some") => {
                        if args.len() != 1 {
                            return Err(CodegenError::new("Option.some() requires exactly 1 argument"));
                        }
                    ...
                    ("Result", "error") => {
                        if args.len() != 1 {
                            return Err(CodegenError::new("Result.error() requires exactly 1 argument"));
                        }
                        let val = self.compile_expr(&args[0].node)?;
        ...
        let func = if let Some(f) = self.functions.get(&func_name) {
            *f
            ...
            return Err(CodegenError::new(format!("Unknown function: {}", func_name)));
        };
        ...
        let args_meta: Vec<BasicMetadataValueEnum> = compiled_args.iter().map(|a| (*a).into()).collect();
    ...
    fn compile_method_call(&mut self, object: &Expr, method: &str, args: &[Spanned<Expr>]) -> Result<BasicValueEnum<'ctx>> {
        ...
        let class_name = class_name.ok_or_else(|| CodegenError::new("Cannot determine object type"))?;
    ...
    fn compile_field(&mut self, object: &Expr, field: &str) -> Result<BasicValueEnum<'ctx>> {
        ...
        let field_idx = *class_info.field_indices.get(field)
        ...
        let field_type = class_info.struct_type.get_field_type_at_index(field_idx).unwrap();
        Ok(self.builder.build_load(field_type, field_ptr, field).unwrap())
    }
    ...
    fn compile_index(&mut self, object: &Expr, index: &Expr) -> Result<BasicValueEnum<'ctx>> {
        ...
        let elem_ptr = unsafe {
            self.builder.build_gep(self.context.i64_type(), obj_ptr, &[idx], "elem").unwrap()
        };

        Ok(self.builder.build_load(self.context.i64_type(), elem_ptr, "load").unwrap())
    }

    fn compile_construct(&mut self, ty: &str, args: &[Spanned<Expr>]) -> Result<BasicValueEnum<'ctx>> {
    ...
    fn compile_print(&mut self, args: &[Spanned<Expr>], newline: bool) -> Result<BasicValueEnum<'ctx>> {
        ...
        for arg in args {
            ...
            self.builder.build_call(printf, &call_args, "printf").unwrap();
        }
        ...
        if newline {
            ...
            self.builder.build_call(printf, &[nl_global.as_pointer_value().into()], "printf").unwrap();
        }
    ...
    fn compile_string_interp(&mut self, parts: &[StringPart]) -> Result<BasicValueEnum<'ctx>> {
        ...
        let buffer_call = self.builder.build_call(malloc, &[buffer_size.into()], "strbuf").unwrap();
        ...
        self.builder.build_call(sprintf, &sprintf_args, "sprintf").unwrap();
    ...
    fn create_option_some(&mut self, value: BasicValueEnum<'ctx>) -> Result<BasicValueEnum<'ctx>> {
        ...
        // Set is_some = 1
        let tag_ptr = self.builder.build_struct_gep(option_type, alloca, 0, "tag").unwrap();
        self.builder.build_store(tag_ptr, self.context.i8_type().const_int(1, false)).unwrap();

        // Set value
        let value_ptr = self.builder.build_struct_gep(option_type, alloca, 1, "value").unwrap();
        self.builder.build_store(value_ptr, value).unwrap();

        Ok(self.builder.build_load(option_type, alloca, "option").unwrap())
    }
    ...
    fn create_option_none(&mut self) -> Result<BasicValueEnum<'ctx>> {
        ...
        // Set is_some = 0
        let tag_ptr = self.builder.build_struct_gep(option_type, alloca, 0, "tag").unwrap();
        self.builder.build_store(tag_ptr, self.context.i8_type().const_int(0, false)).unwrap();

        // Set value to 0 (unused)
        let value_ptr = self.builder.build_struct_gep(option_type, alloca, 1, "value").unwrap();
        self.builder.build_store(value_ptr, self.context.i64_type().const_int(0, false)).unwrap();
    ...
    fn create_result_ok(&mut self, value: BasicValueEnum<'ctx>) -> Result<BasicValueEnum<'ctx>> {
        ...
        // Set is_ok = 1
        let tag_ptr = self.builder.build_struct_gep(result_type, alloca, 0, "tag").unwrap();
        self.builder.build_store(tag_ptr, self.context.i8_type().const_int(1, false)).unwrap();

        // Set ok_value
        let ok_ptr = self.builder.build_struct_gep(result_type, alloca, 1, "ok").unwrap();
        self.builder.build_store(ok_ptr, value).unwrap();

        // Set err_value to null
        let err_ptr = self.builder.build_struct_gep(result_type, alloca, 2, "err").unwrap();
        let null = self.context.ptr_type(AddressSpace::default()).const_null();
        self.builder.build_store(err_ptr, null).unwrap();

        Ok(self.builder.build_load(result_type, alloca, "result").unwrap())
    }
    ...
    fn create_result_error(&mut self, error: BasicValueEnum<'ctx>) -> Result<BasicValueEnum<'ctx>> {
        ...
        // Set ok_value to 0
        let ok_ptr = self.builder.build_struct_gep(result_type, alloca, 1, "ok").unwrap();
        self.builder.build_store(ok_ptr, self.context.i64_type().const_int(0, false)).unwrap();
    ...
    fn create_empty_list(&mut self) -> Result<BasicValueEnum<'ctx>> {
        ...
        let capacity_ptr = self.builder.build_struct_gep(list_type, alloca, 0, "capacity").unwrap();
        self.builder.build_store(capacity_ptr, self.context.i64_type().const_int(initial_capacity, false)).unwrap();

        // Length = 0
        let length_ptr = self.builder.build_struct_gep(list_type, alloca, 1, "length").unwrap();
        self.builder.build_store(length_ptr, self.context.i64_type().const_int(0, false)).unwrap();

        // Allocate data - malloc(capacity * 8) for i64 elements
        let malloc = self.get_or_declare_malloc();
        let size = self.context.i64_type().const_int(initial_capacity * 8, false);
        let call_result = self.builder.build_call(malloc, &[size.into()], "data").unwrap();
        let data_ptr = match call_result.try_as_basic_value() {
            ValueKind::Basic(val) => val,
            _ => panic!("malloc should return a value"),
        };

        let data_ptr_field = self.builder.build_struct_gep(list_type, alloca, 2, "data_ptr").unwrap();
    ...
    fn create_empty_map(&mut self) -> Result<BasicValueEnum<'ctx>> {
        ...
        // Initial capacity = 8
        let initial_capacity: u64 = 8;
        let capacity_ptr = self.builder.build_struct_gep(map_type, alloca, 0, "capacity").unwrap();
        self.builder.build_store(capacity_ptr, self.context.i64_type().const_int(initial_capacity, false)).unwrap();

        // Length = 0
        let length_ptr = self.builder.build_struct_gep(map_type, alloca, 1, "length").unwrap();
        self.builder.build_store(length_ptr, self.context.i64_type().const_int(0, false)).unwrap();
        ...
        let keys_call = self.builder.build_call(malloc, &[size.into()], "keys").unwrap();
        let keys_ptr = match keys_call.try_as_basic_value() {
            ValueKind::Basic(val) => val,
            _ => panic!("malloc should return a value"),
        };
        let keys_field = self.builder.build_struct_gep(map_type, alloca, 2, "keys_ptr").unwrap();
        self.builder.build_store(keys_field, keys_ptr).unwrap();

        let values_call = self.builder.build_call(malloc, &[size.into()], "values").unwrap();
        let values_ptr = match values_call.try_as_basic_value() {
            ValueKind::Basic(val) => val,
            _ => panic!("malloc should return a value"),
        };
        let values_field = self.builder.build_struct_gep(map_type, alloca, 3, "values_ptr").unwrap();
        self.builder.build_store(values_field, values_ptr).unwrap();

        Ok(self.builder.build_load(map_type, alloca, "map").unwrap())
    }

    fn compile_list_method(&mut self, list_name: &str, method: &str, args: &[Spanned<Expr>]) -> Result<BasicValueEnum<'ctx>> {
        ...
        match method {
            "push" => {
                ...
                let length_ptr = self.builder.build_struct_gep(list_type, list_ptr, 1, "len_ptr").unwrap();
                let length = self.builder.build_load(self.context.i64_type(), length_ptr, "len").unwrap().into_int_value();

                // Get data pointer
                let data_ptr_ptr = self.builder.build_struct_gep(list_type, list_ptr, 2, "data_ptr_ptr").unwrap();
            ...
            "length" => {
                let length_ptr = self.builder.build_struct_gep(list_type, list_ptr, 1, "len_ptr").unwrap();
                let length = self.builder.build_load(self.context.i64_type(), length_ptr, "len").unwrap();
                Ok(length)
            }
            "set" => {
                // Get data pointer
    ...
    fn compile_map_method(&mut self, map_name: &str, method: &str, args: &[Spanned<Expr>]) -> Result<BasicValueEnum<'ctx>> {
        ...
        match method {
            "length" => {
                let length_ptr = self.builder.build_struct_gep(map_type, map_ptr, 1, "len_ptr").unwrap();
                let length = self.builder.build_load(self.context.i64_type(), length_ptr, "len").unwrap();
                Ok(length)
            }
            "set" => {
                ...
                let keys_ptr_ptr = self.builder.build_struct_gep(map_type, map_ptr, 2, "keys_ptr_ptr").unwrap();
                let keys_ptr = self.builder.build_load(
                    self.context.ptr_type(AddressSpace::default()),
                    keys_ptr_ptr,
                    "keys"
                ).unwrap().into_pointer_value();

                let values_ptr_ptr = self.builder.build_struct_gep(map_type, map_ptr, 3, "vals_ptr_ptr").unwrap();
                let values_ptr = self.builder.build_load(
                    self.context.ptr_type(AddressSpace::default()),
                    values_ptr_ptr,
                    "vals"
                ).unwrap().into_pointer_value();
                ...
                let key_elem_ptr = unsafe {
                    self.builder.build_gep(self.context.i8_type(), keys_ptr, &[offset], "key_ptr").unwrap()
                };
                ...
                let val_elem_ptr = unsafe {
                    self.builder.build_gep(self.context.i8_type(), values_ptr, &[offset], "val_ptr").unwrap()
                };
                let value = self.compile_expr(&args[1].node)?;
            ...
            "get" => {
                ...
                let length_ptr = self.builder.build_struct_gep(map_type, map_ptr, 1, "len_ptr").unwrap();
                let _length = self.builder.build_load(self.context.i64_type(), length_ptr, "len").unwrap();

                let keys_ptr_ptr = self.builder.build_struct_gep(map_type, map_ptr, 2, "keys_ptr_ptr").unwrap();
                let _keys_ptr = self.builder.build_load(
                    self.context.ptr_type(AddressSpace::default()),
                    keys_ptr_ptr,
                    "keys"
                ).unwrap();
                ...
                let values_ptr_ptr = self.builder.build_struct_gep(map_type, map_ptr, 3, "vals_ptr_ptr").unwrap();
                let _values_ptr = self.builder.build_load(
                    self.context.ptr_type(AddressSpace::default()),
                    values_ptr_ptr,
                    "vals"
                ).unwrap();
    ...
    fn compile_try(&mut self, inner: &Expr) -> Result<BasicValueEnum<'ctx>> {
        ...
        let is_some_or_ok = self.builder.build_int_compare(
            IntPredicate::EQ,
            tag_int,
            self.context.i8_type().const_int(1, false),
            "is_some_or_ok"
        ...
        // Branch based on tag
        self.builder.build_conditional_branch(is_some_or_ok, success_block, error_block).unwrap();
        ...
        match &return_type {
            ...
            Type::Result(ok_ty, err_ty) => {
                ...
                let err_val = self.builder.build_extract_value(struct_val, 2, "err_val").unwrap();
            ...
            _ => {
                return Err(CodegenError::new("? operator can only be used in functions returning Option or Result"));
            }
        }

        // Success block: extract the value and continue
        self.builder.position_at_end(success_block);
        let extracted = self.builder.build_extract_value(struct_val, 1, "unwrapped").unwrap();
        self.builder.build_unconditional_branch(merge_block).unwrap();
    ...
    fn compile_stdlib_function(&mut self, name: &str, args: &[Spanned<Expr>]) -> Result<Option<BasicValueEnum<'ctx>>> {
        match name {
            // Math functions
            "abs" => {
                let val = self.compile_expr(&args[0].node)?;
            ...
            "min" => {
                ...
                if a.is_int_value() {
                    ...
                    let cond = self.builder.build_int_compare(IntPredicate::SLT, av, bv, "cmp").unwrap();
                    let result = self.builder.build_select(cond, av, bv, "min").unwrap();
                    Ok(Some(result))
                } else {
                    let fmin = self.get_or_declare_math_func2("fmin");
                    let call = self.builder.build_call(fmin, &[a.into(), b.into()], "min").unwrap();
                    Ok(Some(self.extract_call_value(call)))
                }
            }
            "max" => {
                ...
                if a.is_int_value() {
                    let av = a.into_int_value();
                    let bv = b.into_int_value();
                    let cond = self.builder.build_int_compare(IntPredicate::SGT, av, bv, "cmp").unwrap();
                    let result = self.builder.build_select(cond, av, bv, "max").unwrap();
                    Ok(Some(result))
                } else {
                    let fmax = self.get_or_declare_math_func2("fmax");
                    let call = self.builder.build_call(fmax, &[a.into(), b.into()], "max").unwrap();
                    Ok(Some(self.extract_call_value(call)))
                }
            }
            "sqrt" => {
                ...
                let call = self.builder.build_call(sqrt, &[fval.into()], "sqrt").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "pow" => {
                ...
                let call = self.builder.build_call(pow_fn, &[fbase.into(), fexp.into()], "pow").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "sin" => {
                let val = self.compile_expr(&args[0].node)?;
                ...
                let call = self.builder.build_call(sin_fn, &[fval.into()], "sin").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "cos" => {
                let val = self.compile_expr(&args[0].node)?;
                ...
                let call = self.builder.build_call(cos_fn, &[fval.into()], "cos").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "tan" => {
                let val = self.compile_expr(&args[0].node)?;
                ...
                let call = self.builder.build_call(tan_fn, &[fval.into()], "tan").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "floor" => {
                let val = self.compile_expr(&args[0].node)?;
                let floor_fn = self.get_or_declare_math_func("floor", true);
                let call = self.builder.build_call(floor_fn, &[val.into()], "floor").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "ceil" => {
                let val = self.compile_expr(&args[0].node)?;
                let ceil_fn = self.get_or_declare_math_func("ceil", true);
                let call = self.builder.build_call(ceil_fn, &[val.into()], "ceil").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "round" => {
                let val = self.compile_expr(&args[0].node)?;
                let round_fn = self.get_or_declare_math_func("round", true);
                let call = self.builder.build_call(round_fn, &[val.into()], "round").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "log" => {
                let val = self.compile_expr(&args[0].node)?;
                ...
                let call = self.builder.build_call(log_fn, &[fval.into()], "log").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "log10" => {
                let val = self.compile_expr(&args[0].node)?;
                ...
                let call = self.builder.build_call(log10_fn, &[fval.into()], "log10").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "exp" => {
                let val = self.compile_expr(&args[0].node)?;
                ...
                let call = self.builder.build_call(exp_fn, &[fval.into()], "exp").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            ...
            "strlen" => {
                ...
                let call = self.builder.build_call(strlen_fn, &[s.into()], "len").unwrap();
                Ok(Some(self.extract_call_value(call)))
            }
            "strcmp" => {
                ...
                let call = self.builder.build_call(strcmp_fn, &[s1.into(), s2.into()], "cmp").unwrap();
                // strcmp returns i32, extend to i64
                let result = self.extract_call_value(call).into_int_value();
                let extended = self.builder.build_int_s_extend(result, self.context.i64_type(), "cmp64").unwrap();
            ...
            "strcat" => {
                ...
                // Get lengths
                let len1_call = self.builder.build_call(strlen_fn, &[s1.into()], "len1").unwrap();
                let len1 = self.extract_call_value(len1_call).into_int_value();
                let len2_call = self.builder.build_call(strlen_fn, &[s2.into()], "len2").unwrap();
                ...
                let buffer_call = self.builder.build_call(malloc, &[buffer_size.into()], "buf").unwrap();
                let buffer = self.extract_call_value(buffer_call).into_pointer_value();

                // strcpy(buffer, s1)
                self.builder.build_call(strcpy_fn, &[buffer.into(), s1.into()], "").unwrap();
                // strcat(buffer, s2)
                self.builder.build_call(strcat_fn, &[buffer.into(), s2.into()], "").unwrap();
            ...
            "read_line" => {
                ...
                let buffer_call = self.builder.build_call(malloc, &[buffer_size.into()], "linebuf").unwrap();
                let buffer = self.extract_call_value(buffer_call).into_pointer_value();
                ...
                self.builder.build_call(
                    fgets,
                    &[buffer.into(), self.context.i32_type().const_int(1024, false).into(), stdin_val.into()],
                    "fgets"
                ).unwrap();
            ...
            "exit" => {
                ...
                let code_i32 = self.builder.build_int_truncate(
                    code.into_int_value(),
    ...
    fn get_or_declare_stdin(&mut self) -> PointerValue<'ctx> {
        ...
        let call = self.builder.build_call(func, &[self.context.i32_type().const_int(0, false).into()], "stdin").unwrap();
    ...
    fn extract_call_value(&self, call: inkwell::values::CallSiteValue<'ctx>) -> BasicValueEnum<'ctx> {
        match call.try_as_basic_value() {
            ValueKind::Basic(val) => val,
    ...
    fn compile_lambda(&mut self, params: &[Parameter], body: &Spanned<Expr>) -> Result<BasicValueEnum<'ctx>> {
        ...
        for (i, param) in params.iter().enumerate() {
            ...
            let alloca = self.builder.build_alloca(self.llvm_type(&param.ty), &param.name).unwrap();
            self.builder.build_store(alloca, llvm_param).unwrap();
            self.variables.insert(param.name.clone(), Variable {
                ptr: alloca,
                ty: param.ty.clone(),
            });
        }
    ...
    fn compile_match_expr(&mut self, expr: &Expr, arms: &[MatchArm]) -> Result<BasicValueEnum<'ctx>> {
    ...
    fn compile_lvalue(&mut self, expr: &Expr) -> Result<PointerValue<'ctx>> {
        match expr {
            ...
            Expr::Field { object, field } => {
                ...
                Ok(self.builder.build_struct_gep(class_info.struct_type, obj_ptr, field_idx, field).unwrap())
            }
